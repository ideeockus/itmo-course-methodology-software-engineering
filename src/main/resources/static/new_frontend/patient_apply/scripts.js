...
//  const jsonData = JSON.stringify(jsonObject);
  ...
  fetch("/patients/create_appointment", {
    method: "post",
    headers: {
    ...
    body: JSON.stringify({
      ...
      appointmentTime: checked_time_slots[0].innerHTML,
  ...
  .then( (response) => {
     ...
     response.json().then(resp_json => {
        ...
        let userToken = resp_json['userToken']
        alert("Ваш персональный токен: " + userToken)
        window.location.replace("patient_profile?token="+userToken);
     })
     .catch(error => {
